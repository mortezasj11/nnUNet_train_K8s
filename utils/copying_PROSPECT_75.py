...
path = '/Data7/JWu11/Sheeba/Sheeba_databases_Final/PROSPECT/PROSPECT_updated_2/'
...
if __name__=='__main__':
    for idx, mrn in enumerate(os.listdir(path)):
        ...
        try:
            ...
            # Segs
            path_tumor_src = os.path.join(path_mrn, 'Seg_tumor_1.nii')
            seg_tumor = nib.load(path_tumor_src).get_fdata()
            seg_tumor[seg_tumor!=0] = 1

            # Destination paths for the new compressed files
            path_ct_des = os.path.join(path_des_im_tr, f'{mrn}_PROS_0000.nii.gz')
            path_seg_des = os.path.join(path_des_lb_tr, f'{mrn}_PROS.nii.gz')
